...
void play (Maze *maze) {
	...
	if (maze->difficulty == 2) {
		...
		nb_spiders = ceil(nb_empty_cells * SPIDER_RATIO);
		ghosts = malloc(nb_ghosts * sizeof(Player));
		spiders = malloc(nb_spiders * sizeof(Player));
		for (i = 0; i < nb_ghosts; i++) {
			ghosts[i] = NULL;
		}
		for (i = 0; i < nb_spiders; i++) {
		    spiders[i] = NULL;
		}
	}
	...
	init_player(&player, PLAYER, 1, 0, 'o');
	set_player(&maze->board[1][0], &player);
	init_ghosts(maze, ghosts, nb_ghosts);
	init_spiders(maze, spiders, nb_spiders);
	...
	do {
		...
		if (res == 0) { /* Update the score only if the player has moved.  */
			...
			if (maze->difficulty == 2) {
				...
				update_spiders(maze, spiders, nb_spiders, player, &score);
			}
		}	
...
void update_spiders(Maze *maze, Player **spiders, int nb_spiders, Player player, int *score) {
    ...
    for (i = 0; i < nb_spiders; i++) {
        if (player.current_row == spiders[i]->current_row && player.current_col == spiders[i]->current_col) {
            *score += SPIDER_SCORE;
        }

        move_spider(maze, spiders[i]);
...
void init_ghosts(Maze *maze, Player **ghosts, int nb_ghosts) {
	...
	if (maze->difficulty == 2) {
		...
		for (i = 0; i < nb_ghosts; i++) {
			...
			} while (maze->board[row][col].type != EMPTY || maze->board[row][col].player != NULL);
...
void init_spiders(Maze *maze, Player **spiders, int nb_spiders) {
    ...
    if (maze->difficulty == 2) {
        ...
        for (i = 0; i < nb_spiders; i++) {
			...
			spiders[i] = malloc(sizeof(*spiders[i]));
			init_player(spiders[i], SPIDER, row, col, 'S');
			set_player(&maze->board[row][col], spiders[i]);
        }
    }
}